...
(async () => {
  ...
  //const browser = await playwright['chromium'].connectOverCDP('ws://0.0.0.0:9222/devtools/browser/52168007-6634-41c9-8f6c-542c92b50b88');
const chrome=await playwright.chromium.connect("http://localhost:9223")
const browser=await chrome.newContext()

const page=await browser.addCookies( [{
  name: 'cookie_name',
  value: 'cookie_value',
  domain: 'google.com',
  path: '/',
...
const goto=await newPage.goto("https://www.amazon.com")
const domTree=await newPage.evaluate(buildDomTreeOverlay, { doHighlightElements: true, focusHighlightIndex: -1, viewportExpansion: 0, debugMode: false })
const launch=chrome.isConnected()
console.log(domTree)
  //await browser.close();
})();
